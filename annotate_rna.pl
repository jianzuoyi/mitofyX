...
sub annotate_rna{
  ...
  my( $infile, $project, $query_taxon, $out_directory, $mt_rna, $seqlen, $file, $RNA_EMAX, $RNA_PMIN, $RNA_MINLEN, $port) = @_;
  ...
  foreach $rna ( @rna_order ){
    ...
    # open( BLASTRUN, "blastall -p blastn -F F -d $mt_rna/$rna/$rna -i $infile  | " );
    open( BLASTRUN, "blastn -query $infile -db $mt_rna/$rna/$rna  | " );
    ...
    open( BLASTOUT, ">$out_directory/$rna.blastn.txt" ); # new
    print BLASTOUT $blast_data;
    close BLASTOUT;
      
    #store locations of html output file for each gene
    if( $rna =~ /fm/i ){
      $rna_html_files{Metf}  = "\.\./$project/" . "$rna.html";
      $rna_blast_files{Metf} = "\.\./$project/" . "$rna.blastn.txt";
    }else{
      $rna_html_files{$rna}   = "\.\./$project/" . "$rna.html";
      $rna_blast_files{$rna}  = "\.\./$project/" . "$rna.blastn.txt";
    }
    ...
    my $rna_bool = 1;
    rna_output_header( $project, $query_taxon, $rna, $gene_length, $out_directory, \@gene_lengths, $rna_bool, $port);
  ...
  parse_trnascan( $infile, $file, $project, $query_taxon, $out_directory, \@missed_by_blast, \@missed_by_tscan, \@missed, \%rna_html_files, \@all_found, \%tscan_html_files, \%tscan_html_paths, $port);
