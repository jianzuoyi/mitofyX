...
sub print_output_header{
  my ( $project, $query_taxon, $gene, $gene_length, $out_directory, $gene_lengths_ref, $rna_bool, $port) = @_;
  ...
  print HTML "\t\t<td><table><form METHOD=\"GET\"><INPUT TYPE=\"button\" VALUE=\"Annotate\" onClick=\"window.open(\'http:\/\/127.0.0.1:$port\/cgi\/make_annotation_form.cgi?gene_name=$gene&taxon=$query_taxon&project=$project\',\'Sequin_form\',\'width=550,height=680,resizable=yes,menubar=no\')\"><\/form><\/table><\/td>\n";
  if( $rna_bool ){
...
####################################################################################################
  sub print_gene_summary{
    ...
    print HTML_SUM "\t\t<td><font face=\"Arial\" color=\"#586cad\" size=\"5\"><a href=\"$rna_output_file\" target=\"_blank\">(see RNA results)</a><\/font><\/td>\n";
    print HTML_SUM "\t<\/tr>\n";
...
sub make_dna_no_hits{
  ...
  $proteinNoHits->{$gene} = "\.\.\/$project" . "/" . $gene . "_NoHits.html";
