...
$out_directory = ($OUTDIR or "blast_output")."/$project"; 
...
$mt_genes = ($GENEDB or "blast_dbs/mt_genes"); 
$mt_rna = ($RNADB or "blast_dbs/mt_rna"); 

print "\nNeither Gene or RNA database found !" and exit unless ((-e "$mt_genes" and !is_folder_empty($mt_genes)) || (-e "$mt_rna" and !is_folder_empty($mt_rna)));

opendir(my $DH, $mt_genes) or die "$mt_genes not a directory";

my @tglist = grep {-d "$mt_genes/$_" && ! /^\.{1,2}$/} readdir($DH);
my %tglist = map { $_ => 1 } @tglist;
my @gene_list = qw( atp1 atp4 atp6 atp8 atp9 ccmB ccmC ccmFc ccmFn cob cox1 cox2 cox3 matR mttB nad1 nad2 nad3 nad4 nad4L nad5 nad5_ex3 nad6 nad7 nad9 rpl2 rpl5 rpl10 rpl16 rps1 rps2 rps3 rps4 rps7 rps10 rps11 rps12 rps13 rps14 rps19 sdh3 sdh4 );
my @glist = grep { $tglist{$_} } @gene_list;
my @failed_list = grep { ! $tglist{$_} } @gene_list;

print "\n\nIgnoring genes not found in your database : \n";
print "    - ". join(", ", @failed_list) ."\n";


print "\n\nBLASTing remaining genes . . . \n";

foreach $gene ( @glist ){
  $gene =~ /^\./ and next; #ignores the "." and ".." elements in the directory
  # $gene eq "ccmFc" or next;
  # check if database was constructed, otherwise, rebuild it
  ...
  if( $gene eq "nad5_ex3" ){
    ...
    open( BLASTOUT, ">$out_directory/$gene.blastn.txt" );	#NEW
    ...
    #save raw BLAST output
    open( BLASTOUT, ">$out_directory/$gene.blastx.txt" );	#NEW
    print BLASTOUT $blast_data;
    close BLASTOUT;
    
    #store locations of html and raw blast output files for each gene
    $html_files{$gene} = "\.\.\/$project" . "/" . "$gene.html";
    $blast_files{$gene} = "\.\.\/$project" . "/" . "$gene.blastx.txt";
    ...
    print_output_header( $project, $query_taxon, $gene, $gene_length, $out_directory, \@gene_lengths, $rna_bool, $port);
...
annotate_rna( $infile, $project, $query_taxon, $out_directory, $mt_rna, $seqlen, \$file, $RNA_EMAX, $RNA_PMIN, $RNA_MINLEN, $port);
print_gene_summary( $project, $query_taxon, $seqlen, \%html_files, \%blast_files, $out_directory, \@missed );

print "\n\n** Open \'$out_directory/$project\_summary.html\' and \'$out_directory/$project\_rna\_summary.html\' in a web browser to see results. **\n\n";
