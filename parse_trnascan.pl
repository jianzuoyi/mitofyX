...
sub parse_trnascan{
  my( $infile, $file_ref, $project, $query_taxon, $out_directory, $missed_by_blast_ref, $missed_by_tscan_ref, $missed_ref, $rna_html_files_Href, $all_found_ref, $tscan_html_files_Href, $tscan_html_paths, $port ) = @_;
  my $outfile;
  ...
  unless( -e "$out_directory/$tscan_out" ){
    ...
    system( "tRNAscan-SE -q -O -o $out_directory/$tscan_out -f $out_directory/$tscan_struct_out $infile" );
  }
  ...
  while( <TSCAN> ){
    if( /^($query_taxon)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)/ ){
      ...
      $tscan_html_paths->{$5}      = "\.\.\/$project" . "/$5" . "_trnascan.html";
    }
  }
  close TSCAN;
  ...
  while( <TSCAN> ){
    ...
    print_trnascan( $project, $missed_ref, \%trna_letters, $file_ref, $query_taxon, $out_directory, $start, $end, $aa, $anti, $intron_start, $intron_end, $cove_score, $tscan, $port );
  }
...
sub print_trnascan{
  my( $project, $missed_ref, $trna_letters_ref, $file_ref, $query_taxon, $out_directory, $start, $end, $aa, $anti, $intron_start, $intron_end, $cove_score, $tscan, $port) = @_;
  ...
  open( HTML,">>$outfile" ) || die "Couldn't open $outfile: $!\n";
  print_trn_title( $project, $query_taxon, $trna_letters_ref, $aa, $anti, $length, $coords, $port);
...
sub print_trn_title{
  my( $project, $query_taxon, $trna_letters, $aa, $anti, $length, $coords, $port ) = @_;
  my $gene_name;
  ...
  if( $aa eq "Undet" ){
    ...
    print HTML "<table valign=\"middle\">\n\t<tr nowrap valign=\"middle\">\n";
    print HTML "\t\t<td><font face=\"Times\">", '&nbsp;' x 85, "<\/font><\/td>\n";
    print HTML "\t\t<td><table><form METHOD=\"GET\"><INPUT TYPE=\"button\" VALUE=\"Annotate\" onClick=\"window.open(\'http:\/\/127.0.0.1:$port\/cgi\/trnC_make_annotation_form.cgi?gene_name=$gene_name&gene_coords=$coords&product=tRNA-$aa&note=anticodon:$anti&taxon=$query_taxon&project=$project\',\'Sequin_form\',\'width=550,height=600,resizable=yes,menubar=no\')\"><\/form><\/table><\/td>\n";
    print HTML "\t\t<td><font face=\"Courier\" color=\"blue\" size=\"6\">&nbsp;&nbsp;&nbsp;$gene_name&nbsp;($length&nbsp;nt)<\/font><\/td>\n";
    print HTML "\t<\/tr>\n<\/table>\n<br><br>\n";
